...
class EmployerControllerTest {
  ...
  void testGetEmployerById() throws Exception {
    api.perform(get("/employers/1")).andExpect(status().isOk());
  }

  @Test
  @WithMockUser(roles = "USER")
  void testGetEmployerByIdWithUser() throws Exception {
    api.perform(get("/employers/1")).andExpect(status().isForbidden());
  }

  @Test
  void testGetEmployerByIdWithoutAuthen() throws Exception {
    api.perform(get("/employers/1")).andExpect(status().isUnauthorized());
  }
}
