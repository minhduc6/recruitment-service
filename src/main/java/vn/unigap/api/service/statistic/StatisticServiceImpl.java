...
public class StatisticServiceImpl implements StatisticService{
    ...
    public StatisticTheSystemDTO generalIndexOfTheSystemByDay(String fromDate, String toDate) {
        String sql = "SELECT formatted_date, SUM(count_employer) AS total_count_employer, SUM(count_job) AS total_count_job, SUM(count_seeker) AS total_count_seeker, SUM(count_resume) AS total_count_resume " +
                "FROM ( " +
        ...
        for (Map<String, Object> row : results) {
            ...
            chart.add(new StatisticByDay(LocalDate.parse(formattedDate),totalCountEmployer,totalCountJob,totalCountSeeker,totalCountResume));
        }
        // Sort day
        chart.sort(Comparator.comparing(StatisticByDay::getDay));
        return  new StatisticTheSystemDTO(totalCountEmployerSystem,totalCountJobSystem,totalCountSeekerSystem,totalCountResumeSystem,chart);
    }
}
