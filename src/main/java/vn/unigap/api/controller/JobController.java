...
public class JobController {
    ...
    public ResponseWrapper<PagingWrapper<?>> getAllJobByEmployerId(@RequestParam(required = false, defaultValue = "1") @Parameter(example = "1") Integer page, @RequestParam(required = false, defaultValue = "10") @Parameter(example = "10") Integer size, @RequestParam(required = false, defaultValue = "-1") @Parameter(example = "-1") Integer employerId) {
        ...
        logger.info("Received request: GET /jobs?page={}, size={}, employerId={}", page, size, employerId);
        ...
        logger.info("Returning response: GET /jobs?page={}, size={} , employerId={},{} ", page, size, employerId, pagingWrapper);
        return new ResponseWrapper<>(0, HttpStatus.OK.value(), "Get jobs successfully", pagingWrapper);
    }

    @GetMapping("/job-information")
    public ResponseWrapper<JobInformation> getJobInformationAndSeekerRightFit(@RequestParam(required = false, defaultValue = "-1") @Parameter(example = "-1") Integer jobId) {
        ...
        return new ResponseWrapper<>(0, HttpStatus.OK.value(), "Get Job Information successfully " + jobId, jobInformation);
    }
    ...
    public ResponseWrapper<JobByIdDTO> getJobById(@PathVariable Integer id) {
        ...
        return new ResponseWrapper<>(0, HttpStatus.OK.value(), "Get Job successfully " + id, jobByIdDTO);
    }
    ...
    public ResponseWrapper<JobDTO> updateJob(@PathVariable Integer id, @Valid @RequestBody UpdateJobRequest updateJobRequest) {
        logger.info("Received request PUT /jobs to update job with ID: {}. Payload: {}", id, updateJobRequest);
