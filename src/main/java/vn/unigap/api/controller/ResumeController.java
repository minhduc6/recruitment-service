...
public class ResumeController {
  ...
  @GetMapping
  public ResponseWrapper<PagingWrapper<?>> getAllResumeBySeekerId(
      ...
      @RequestParam(defaultValue = "-1") Integer seekerId) {
    int adjustedPage = page - 1;
    logger.info(
        "Received request: GET /resumes?page={}, size={}, seekerId={}", page, size, seekerId);
    Page<ResumeDTO> paginatedData =
        resumeService.getAllResumeBySeekerId(adjustedPage, size, seekerId);
    ...
    PagingWrapper<?> pagingWrapper =
        new PagingWrapper<>(
            ...
            resumeDTOS);
    logger.info(
        "Returning response: GET /resumes?page={}, size={} ,seekerId={}  {} ",
        page,
        size,
        seekerId,
        pagingWrapper);
    return new ResponseWrapper<>(
        0, HttpStatus.OK.value(), "Get resumes successfully", pagingWrapper);
  }
  ...
  public ResponseWrapper<ResumeByIdDTO> getResumeById(@PathVariable Integer id) {
    ...
    return new ResponseWrapper<>(
        0, HttpStatus.OK.value(), "Get Resume successfully " + id, resumeByIdDTO);
  }

  @PostMapping
  @ResponseStatus(HttpStatus.CREATED)
  public ResponseWrapper<ResumeDTO> createResume(
      @Valid @RequestBody CreateResumeRequest createResumeRequest) {
    logger.info(
        "Received request POST /resumes to create resume. Payload: {}", createResumeRequest);
    resumeService.createResume(createResumeRequest);
    logger.info("Returning response POST /resumes Resume created successfully");
    return new ResponseWrapper<>(
        0, HttpStatus.CREATED.value(), "Resume created successfully", null);
  }

  @PutMapping("/{id}")
  public ResponseWrapper<ResumeDTO> updateResume(
      @PathVariable Integer id, @Valid @RequestBody UpdateResumeRequest updateResumeRequest) {
    logger.info(
        "Received request PUT /resumes to update resume with ID: {}. Payload: {}",
        id,
        updateResumeRequest);
